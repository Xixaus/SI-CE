...
class ChemstationCommunicator:
    ...
    def __init__(self, config: Optional[CommunicationConfig] = None):
        ...
        self.comm_macro_path = self.config.get_chempy_connect_path()
    ...
    def _macro_update_paths(self) -> None:
        """Update communication directory path in ChemPyConnect macro.
        
        Reads the ChemPyConnect.mac file and replaces the hardcoded communication
        directory path in the CP_Loop macro with the current path from configuration.
        This ensures the macro always monitors the correct directory regardless of
        where the project is installed.
        
        The function specifically targets the MonitorFile command in the CP_Loop
        macro and updates its path argument to match the current communication
        directory from the configuration.
        
        Path Format:
            - Converts forward slashes to backslashes for Windows compatibility
            - Maintains the double-quote wrapping required by macro syntax
            - Example: MonitorFile "c:\\Users\\user\\project\\communication_files"
        
        Raises:
            FileOperationError: If the macro file cannot be read or written.
                            This typically indicates permission issues or
                            missing macro file.
                            
        Note:
            This method should be called during initialization before the macro
            is loaded into ChemStation. The macro must be reloaded in ChemStation
            after path updates for changes to take effect.
        """
        try:
            # Read current macro content
            with open(self.comm_macro_path, 'r', encoding='utf-8') as f:
                content = f.read()
            
            # Convert path to Windows format with escaped backslashes
            comm_path = str(self.comm_dir).replace('/', '\\')
            
            # Find and replace the path in MonitorFile command
            # Look for pattern: MonitorFile "any_path"
            pattern = r'MonitorFile\s+"[^"]*"'
            # Use lambda function to avoid escaping issues with backslashes
            replacement = lambda m: f'MonitorFile "{comm_path}"'
            
            updated_content = re.sub(pattern, replacement, content)
            
            # Write updated content back
            with open(self.comm_macro_path, 'w', encoding='utf-8') as f:
                f.write(updated_content)
                
            if self.config.verbose:
                print(f"✓ Updated macro path to: {comm_path}")
                
        except IOError as e:
            raise FileOperationError(f"Failed to update macro paths: {e}")
