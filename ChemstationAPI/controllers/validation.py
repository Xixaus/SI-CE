...
class ValidationModule():
    ...
    def validate_method(self, method: str, dir_path: str = "_METHODPATHS$", check_vials: bool = False) -> None:
        """Validate that CE method file exists in specified directory.
    
        Args:
            method: Method name (without .M extension).
            dir_path: Path to method directory. Defaults to _METHPATH$.
            check_vials: If True, also validates that method's vials are in carousel.
        
        Raises:
            ValidationError: If method file doesn't exist.
            VialError: If check_vials=True and required vials are missing.
        """
        method_upper = (method + ".M").upper()
        
        if dir_path == "_METHODPATHS$":
            dir_path = Path(self.comm.send("response$ = _METHODPATHS$", timeout=15))
        
        if not os.path.exists(dir_path):
            raise FileNotFoundError(f"Method directory not found: {dir_path}")
        
        filenames_upper = [file.upper() for file in os.listdir(dir_path)]
        if method_upper not in filenames_upper:
            raise ValidationError(f"Method '{method}' not found in directory {dir_path}")
        
        # Volitelná validace vialek
        if check_vials:
            self.validate_vials_in_method(method, dir_path)

    def validate_vials_in_method(self, method: str = "_METHFILE$", dir_path: str = "_METHODPATHS$") -> None:
        """Validate that all vials required by the method are present in carousel.
        
        Args:
            method: Method name (without .M extension) or "_METHFILE$" for currently loaded method.
            dir_path: Path to method directory. Defaults to _METHODPATHS$.
            
        Raises:
            VialError: If required vials are missing from carousel.
        """
        # Zjisti aktuálně načtenou metodu pokud je použito _METHFILE$
        if method == "_METHFILE$":
            current_method_full = self.comm.send("response$ = _METHFILE$", timeout=15).strip()
            if not current_method_full:
                raise ValidationError("No method is currently loaded")
            # Odstraň cestu a příponu .M pro získání názvu metody
            method = Path(current_method_full).stem
        
        # Zjisti adresář metod pokud je použito _METHODPATHS$
        if dir_path == "_METHODPATHS$":
            dir_path = Path(self.comm.send("response$ = _METHODPATHS$", timeout=15))
        ...
        # Najdi skutečný název souboru (zachová původní velikost písmen)
        filenames_upper = [file.upper() for file in os.listdir(dir_path)]
        if method_upper not in filenames_upper:
            raise ValidationError(f"Method '{method}' not found in directory {dir_path}")
        
        actual_filename = next(f for f in os.listdir(dir_path) if f.upper() == method_upper)
        method_path = os.path.join(dir_path, actual_filename)
        
        # Extrahuj vialky z XML souboru metody
        required_vials = self.extract_vials_from_xml(method_path)
        
        if required_vials:
            self.list_vial_validation(required_vials)
    ...
    def validate_use_carousel(self, num_attempt=3) -> None:
        ...
        Args:
            num_attempt (int): Number of validation attempts before raising error.
                            Default is 2. Each attempt is separated by 2 second delay.
    
        Raises:
            SystemError: If carousel is not available for use after all attempts.
        ...
        raise SystemError(
            f"Carousel not available for use after {num_attempt} attempts - "
            f"final instrument state: {last_state}. Allowed states: {allowed_states}"
        )
    ...
    def extract_vials_from_xml(self, method_path):
        """Extract vial numbers from CE method XML configuration file.
        
        Parses ChemStation CE method XML files to extract all vial position
        numbers referenced in the method configuration. This includes vials
        used for inlet/outlet positions, preconditioning programs, injection
        programs, and any other method operations requiring vial access.
        
        Args:
            method_path (str): Path to the CE method directory (.M folder).
                            Function automatically looks for the standard
                            "AgilentCEDriver1.RapidControl.MethodXML.xml" file
                            within this directory.
                            
        Returns:
            list: Sorted list of unique vial position numbers (integers).
                Only positive vial numbers are included (negative values like -1
                are filtered out as they represent "no vial" conditions).
                Empty list if no valid vials found or file reading fails.
        ...
        Note:
            ...
            - Searches for all <Vialnumber></Vialnumber> tags in the XML
            - Automatically filters negative vial numbers (-1, etc.)
        ...
        """
        try:
            # Construct path to the standard XML file
            xml_file_path = os.path.join(method_path, "AgilentCEDriver1.RapidControl.MethodXML.xml")
            
            with open(xml_file_path, 'r', encoding='utf-16') as f:
                content = f.read()
            
            pattern = r'<Vialnumber>([^<]+)</Vialnumber>'
            matches = re.findall(pattern, content)
            
            vial_numbers = []
            for match in matches:
                try:
                    number = int(match.strip())
                    if number >= 0:
                        vial_numbers.append(number)
                except ValueError:
                    continue
            
            return sorted(list(set(vial_numbers)))
            
        except Exception as e:
            print(f"Error reading method XML: {e}")
            return []